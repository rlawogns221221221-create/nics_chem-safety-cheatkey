...
"""정리한 표들을 **한 화면에서 훑어볼 수 있는 HTML** 한 장으로 굽는다.
   ...
   왜 HTML 인가 — 받는 사람이 개발자가 아닙니다. CSV 14개를 엑셀로 하나씩
   열어 보게 하면 "무엇이 얼마나 있는지"를 파악하는 데만 반나절이 걸립니다.
   파일 하나를 더블클릭하면 표 14개를 오가며 검색까지 되게 합니다.
...
카드 = [
    ('바로 쓸 수 있는 지점', '1,141', '주소·전화가 있어 지도에 찍을 수 있는 곳'),
    ('지금 서비스에 있는 것', '470', '2026-08-11 기준 · 약 2.4배가 됩니다'),
    ('건수만 확인', '1,430', '지정폐기물 처리업체 — 행 단위 재취합 필요'),
    ('걷어낸 개인정보', '7,252', '실명 2,480 · 휴대전화 495 · 사업자번호 4,277'),
]
...
<header>
  <h1>방제자원 원자료 정리</h1>
  <p>구글드라이브 <b>방제자원_전체</b> 폴더 → 표 14개 · 자료 7,469행 ·
...
경고문 = {
    ...
    '12_지정폐기물_처리업체_유역청별_건수.csv': ('유역청 7곳이 서로 다른 양식으로 내서 '
                                     '<b>건수로만</b> 정리했습니다. 대구청 1곳은 아직 못 읽었습니다.', False),
