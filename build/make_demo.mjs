...
/* 이 영상에서 쓰는 사고지점 — 우리 자료에 실제로 있는 사업장입니다
   ...
   ②③ 모두 **이 이름 하나로** 넣습니다(2026-09-14 사용자 지시로 통일).
   ② 도 찾을 수 있게 된 것은 data/places.js 가 생겼기 때문입니다. */
const 사업장 = 'SK하이닉스';
...
/* ══ 1. 01 주민 대피장소 찾기 ══════════════════════════════ */
/* ⚠ 차례는 **진입 화면의 카드 차례**와 같아야 합니다(2026-09-14 사용자 지시로
   방제가 다시 3번이 되었습니다). 첫 도구만 카드를 눌러 들어가고, 뒤의 둘은
   제목 카드를 띄운 뒤 주소로 넘어갑니다 — 진입 화면을 세 번 거치면 늘어집니다. */
await 자막('① 주민 대피장소 찾기 — 사고지점만 넣으면 가까운 순으로 나옵니다', 1800);
await 가서누르기('.pn', { i: 0, 뒤: 1200 });
await 잠깐(500);
await 자막자리();
/* ⚠ 세 장면 모두 **같은 사고지점(SK하이닉스㈜ 청주)** 을 같은 방법으로
   넣습니다(2026-09-14 사용자 지시 — "다른 장면과 통일"). 예전에는 ② 만
   `청주` 로 시·군·구를 골랐는데, ② 의 검색이 대피장소 이름·시군구·읍면동만
   찾았기 때문입니다. 이제 `data/places.js` 로 사업장 이름도 찾습니다. */
await 자막('사고지점은 사업장 이름으로 넣습니다 — 예) SK하이닉스 청주', 2000);
await 이름으로찍기('#mAddrQ', 사업장, 'SK하이닉스');
await 타일기다리기();
await 예시띠가리기();
await 자막자리();
...
/* ══ 3. 03 방제 물품·장비 찾기 ═════════════════════════════ */
await 제목('방제 물품·장비 찾기', '업체 1,717건 · 물품 4,075건을 가까운 순으로', 1600);
await 제목(null, null, 200);
await p.goto(`${ROOT}res/index.html`);
await 잠깐(1000);
await 자막자리();
/* ⚠ 자막 두 줄을 잇달아 띄우면 그동안 화면이 멈춰 보입니다(사용자 지적 —
   "중간중간 정적이 흐르는 지점"). 사이에 손놀림을 한 번 넣습니다. */
await 자막('③ 방제 물품·장비 찾기 — 무엇이 어디에 있는지 찾습니다', 1500);
await 훑기('.rz-br', 500);
