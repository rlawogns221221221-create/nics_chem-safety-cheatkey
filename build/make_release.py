...
`화학사고_초동대응_지원_서비스_사용설명서.pdf` 는 **지자체 담당자에게 그대로
보내는 설명서**입니다. 처음 접하는 사람이 읽도록 쉬운 말로 썼고, 실제 화면
사진이 들어 있습니다 — 접속 방법(PC·안드로이드·아이폰), 바탕화면 아이콘
만들기, 도구 세 가지 사용법, 잘 안 될 때, 자료 출처가 들어 있습니다.
...
def main() -> None:
    ...
    # 옮기면 됩니다).
    guide = ROOT / "docs" / "화학사고_초동대응_지원_서비스_사용설명서.pdf"
    if guide.exists():
        shutil.copy2(guide, OUT / guide.name)
