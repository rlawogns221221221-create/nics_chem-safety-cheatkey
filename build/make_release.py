...
# 화면이 실제로 부르는 것만
#
# manifest.webmanifest · sw.js 는 **사이트 뿌리에 있어야** 합니다.
#   · 매니페스트의 start_url·scope 가 자기 위치를 기준으로 잡힙니다.
#   · 서비스워커는 자기가 놓인 폴더까지만 맡습니다. assets/ 안에 두면
#     sms/·map/·res/ 가 그 관할 밖이 되어 도구 세 개가 오프라인에서
#     열리지 않습니다.
# 하위 폴더에 옮기지 마세요.
...
- **안드로이드 휴대전화** — 크롬으로 열고 화면 아래쪽 **‘바탕화면에 추가’**
- **아이폰·아이패드** — **사파리**로 열고 아래쪽 공유 단추(□↑) →
  **‘홈 화면에 추가’** (크롬이 아니라 사파리여야 합니다)
- **업무용 PC** — 크롬·엣지로 열고 화면 아래쪽 ‘바탕화면에 추가’ 또는
  주소창 오른쪽 끝의 설치 아이콘

깔고 나면 **인터넷이 끊겨도 화면이 뜹니다**(한 번 연 뒤부터). 거리 계산·
문안 만들기·자료 목록은 원래 인터넷 없이 도는 기능입니다. 끊긴 동안에는
배경지도·주소검색·도보경로만 빠집니다.

자료를 고쳐 다시 올리면 담당자가 **다음에 열 때** 반영됩니다. 한 번에 확실히
갈아 끼우려면 `{SITE_NAME}/sw.js` 맨 위의 `var 버전 = "v1"` 을 `"v2"` 처럼
올린 뒤 다시 올리세요.

> `sw.js` 와 `manifest.webmanifest` 는 **폴더 맨 위(index.html 옆)에**
> 있어야 합니다. 하위 폴더로 옮기면 도구 세 개가 끊겼을 때 열리지 않습니다.
