...
function 알림그리기(){
  var s = D.통계, h = "";
  if (지금 === "업체") {
    h = '<div class="note"><h2>업체 — 아직 못 채운 것</h2>' +
      '<p><b>처리가능 지정폐기물 목록이 ' + s.업체.처리가능폐기물_없음_필수갈래 +
      '곳에 없습니다.</b> 분류안이 “허가증 상에 처리가능한 지정폐기물 목록이 ' +
      '보여야 함”이라고 못 박은 항목입니다. 지금 그 목록이 들어 있는 자료는 ' +
      '두 개뿐이고(표 03 · 13), 가장 넓은 <b>전국취합본(1,511행)에는 그 칸이 ' +
      '아예 없습니다.</b> 허가증 파일(드라이브 <code>0. 폐기물 처리업체</code>)을 ' +
      '풀어야 채울 수 있습니다.</p>' +
      '<p><b>‘중간처분(중화)’ 은 지금 자료로 가려낼 수 없습니다.</b> 원자료의 ' +
      '처리방법 값이 소각·매립·재활용·수집운반·기타 다섯뿐이고 ‘중화’가 ' +
      '없습니다. 소각이 아닌 중간처분은 <b>중간처분(그 외)</b> 로 두고 처리방법 ' +
      '원문을 함께 적었습니다 — 지어내서 ‘중화’라고 적지 않았습니다.</p>' +
      '<p><b>독성가스 회수업체 2곳</b>은 분류안의 여섯 갈래에 없어 ' +
      '<b>특수처리</b>로 따로 두었습니다. 어디에 넣을지 정해 주세요.</p></div>';
  } else {
