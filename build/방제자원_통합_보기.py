 ...
 *{box-sizing:border-box}
 ...
 /* 미리 협의된 곳 — 갈래마다 맨 위에 오고, 한눈에 띄어야 합니다.
    "협의 완료" 라는 말은 담당자가 뜻을 모른다고 해서(2026-09-07 사용자)
    **먼저 연락 · 미리 협의된 곳** 으로 풀어 적었습니다. */
 .row.first{background:var(--ok-l);border-left:4px solid var(--ok);padding-left:11px}
 .row .tag.first{background:var(--ok);color:#fff;font-size:12px}
 .row.first .nm{font-size:16.5px}
...
function 알림그리기(){
  ...
  if (지금 === "업체") {
    h = '<div class="note"><h2>업체 — 무엇이 채워졌고 무엇이 남았나</h2>' +
      '<p><b>초록 띠가 붙은 줄이 먼저 연락할 곳입니다</b>(' + s.업체.협의완료 +
      '줄 · 업체 19곳). <b>미리 협의가 끝나 있고</b> 보내 주신 허가증으로 ' +
      '확인한 곳이라, 갈래마다 <b>맨 위</b>에 올려 두었습니다. 허가증이 ' +
      '스캔 이미지라 글자를 뽑아낼 수 없어 쪽마다 그림으로 읽었습니다.</p>' +
      '<p><b>중간처분은 하나로 합쳤습니다</b>(말씀대로) — 소각·중화·그 외를 ' +
      '나누지 않습니다. 소각인지 중화인지는 줄의 <b>허가</b> 줄에 원문이 ' +
      '그대로 있습니다(예: “지정폐기물 중간처분업(화학적 처분)”). ' +
      '(주)엔아이티의 화학적 처분 허가증에는 처리가능 폐기물이 ' +
      '<b>폐산·폐알칼리</b>로 적혀 있습니다.</p>' +
    ...
    h = '<div class="note"><h2>물품 — 이름을 어떻게 묶었나</h2>' +
      '<p>원자료의 물품 이름이 <b>한 표에서만 484가지</b>였습니다. 처음에 ' +
      '84가지로 묶었는데 <b>칩이 여섯 줄</b>이 되어 훑을 수가 없었습니다 — ' +
      '말씀대로 <b>' + s.물품.물품명수 + '가지</b>로 더 줄였습니다.</p>' +
      '<p>묶는 기준은 <b>급할 때 한 번에 요청하는 단위</b>입니다 — 방독면·' +
      '정화통·공기호흡기는 따로 부르지 않고 “호흡보호구” 로 한꺼번에 ' +
      '부릅니다. <b>세부 종류는 각 줄의 “원자료 표기” 에 그대로</b> 있습니다 ' +
      '(“화학물질 흡착포(패드형 100매)”). 다만 <b>뜻이 다른 것은 크기 때문에 ' +
      '합치지 않았습니다</b> — 흡착포와 중화제를 합치면 폐산을 중화할 것을 ' +
      '찾는 사람이 흡착포를 받습니다.</p>' +
      '<p><b>기타 ' + D.통계.물품.분류별.기타 + '줄</b>은 원자료 자체가 ' +
      '뭉뚱그린 표기입니다(“방제장비”·“소형”·“처리능력”·“물 적재량”). ' +
      '한 칸에 모아 두고 원래 표기는 줄에 남겼습니다.</p>' +
...
function 그리기(){
  ...
  for (var i = 0; i < 끝; i++) {
    var r = 남[i];
    if (지금 === "업체") {
      h += '<div class="row' + (r[칸(칸들,"협의권역")] ? " first" : "") +
        '"><div class="t">' +
        '<span class="tag">' + r[칸(칸들,"갈래")] + '</span>' +
        '<span class="nm">' + r[칸(칸들,"업체명")] + '</span>' +
        (r[칸(칸들,"협의권역")] ? '<span class="tag first">먼저 연락 · ' +
          '미리 협의된 곳</span>' : "") +
        ...
        (r[칸(칸들,"주소")] || "주소 없음") + '</p>' +
        (r[칸(칸들,"협의권역")]
          ? '<p class="m"><b>미리 협의가 끝난 업체입니다</b> — 맡기로 한 권역 ' +
            r[칸(칸들,"협의권역")] + ' · 허가증으로 확인했습니다</p>' : "") +
