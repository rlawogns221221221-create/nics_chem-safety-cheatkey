...
# ② 주민 대피장소 찾기가 읽는 사업장 이름 목록(이름·시도·시군구·좌표만).
# ③ 이 읽는 resources2.js 를 통째로 ② 에 물리면 954KB 가 그대로 따라와
# 쓰지도 않는 업체·물품 줄까지 실립니다 — 필요한 칸만 따로 뽑습니다.
PLACES = ROOT / "data" / "places.js"
...
def main():
    ...
    # ── ② 가 읽는 사업장 이름 목록 ──────────────────────────────
    # ② 주민 대피장소 찾기의 검색은 그동안 **대피장소 이름·시군구·읍면동**
    # 만 찾았습니다. 그런데 그 칸의 안내문은 "사업장·건물 이름 … 으로
    # 찾습니다" 라고 적혀 있었습니다 — 인터넷(브이월드)이 될 때만 참인 말이라,
    # 망분리 PC 에서는 사업장 이름을 쳐도 아무것도 안 나왔습니다.
    # 여기서 **이름·시도·시군구·좌표만** 뽑아 작은 파일로 만들어 ② 가 읽게
    # 합니다(2026-09-14 사용자 결정).
    #   · 전화·주소·보유물품은 넣지 않습니다 — ② 가 쓰지 않고, 파일만 커집니다.
    #   · 좌표가 없는 자리는 넣지 않습니다(2절 3항 — 지도에 찍지 않습니다).
    #   · `ap`(좌표 정확도)를 함께 넘깁니다. 어림값이면 ② 가 "대략" 이라
    #     적고 지도를 한 번 눌러야 사고지점이 확정됩니다.
    자리목록 = [
        {"n": r["n"], "sd": r["sd"], "sg": r["sg"],
         "la": r["la"], "lo": r["lo"], "ap": r.get("ap", "")}
        for r in 자리.rows if r.get("la") is not None and r.get("lo") is not None
    ]
    PLACES.write_text(
        "/* 사업장·기관 이름 목록 — ② 주민 대피장소 찾기의 검색이 읽습니다.\n"
        "   방제자원 자리(RES2_PLACE)에서 **이름·시도·시군구·좌표**만 뽑은 것입니다.\n"
        "   전화·주소·보유물품은 들어 있지 않습니다(② 가 쓰지 않습니다).\n"
        "   담당자 개인정보도 물론 없습니다.\n"
        "\n"
        "   ※ 좌표는 주소로 잡은 어림값입니다(ap 가 그 단위). 고른다고 곧바로\n"
        "     사고지점이 찍히지 않고, 옮겨간 자리에서 지도를 한 번 눌러야\n"
        "     확정됩니다 — 어림값을 실측처럼 보이게 하지 않기 위해서입니다.\n"
        "   ※ 자동 생성 파일입니다. build/make_resources2.py 로 다시 만드세요.\n"
        f"   만든 날: {오늘} · {len(자리목록):,}곳 */\n"
        + js("PLACES", 자리목록),
        encoding="utf-8")
