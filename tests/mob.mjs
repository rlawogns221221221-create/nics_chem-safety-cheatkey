...
for (const [dev, cfg] of DEVICES) {
  ...
  for (const pname of ['진입화면', '① 문자작성', '② 대피장소', '③ 방제 동원']) {
    ...
    if (pname === '진입화면') {
      ...
      checks++;
      if (isRes) {
        /* ③ 은 시작이 세 걸음 — 지도·조건은 사고지점을 정한 뒤에 나온다.
           걸음 1(무엇을: 두 갈래)·걸음 2(필요한 것)·걸음 3(사고지점)을
           각각 재고 지나갑니다. */
        const dead = await P.evaluate(DEADTOP, '.rz');
        if (!wide && dead > cfg.viewport.height * 0.2)
          note(dev, pname, `시작 화면 위쪽이 ${dead}px 비어 있다 (세로 가운데 정렬)`);
        report(dev, pname + '(걸음1 두 갈래)', await P.evaluate(PROBE));
        const br = await P.$('.rz-br');
        if (br) { await br.click(); await P.waitForTimeout(400); }
        report(dev, pname + '(걸음2 필요한 것)', await P.evaluate(PROBE));
        ...
        report(dev, pname + '(걸음3 사고지점)', await P.evaluate(PROBE));
