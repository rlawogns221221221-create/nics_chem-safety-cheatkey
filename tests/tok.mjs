...
/* 요소 하나에만 매달아 쓰는 이름 — 뿌리(:root)에 없는 것이 정상이므로 여기
   적어 검사에서 뺍니다. 지금은 하나도 없습니다 — 진입 화면에서 카드마다 다른
   색을 쓰던 `--pn-c`·`--pn-t` 가 그 예였는데, 개선(안)에 맞춰 세 카드를 같은
   파랑으로 통일하면서 없어졌습니다. 새로 그런 이름을 만들면 여기 적으세요. */
const SCOPED = []
